/*
 * Copyright (C) 2014 Yuya Tanaka Licensed under the Apache License, Version 2.0 (the "License"); you may not use this file
...
public class MediaExtractorUtils {
	...
	public static MandatoryKv getInputMandatoryValue(MediaMetadataRetriever retriever) {
		MandatoryKv kv = new MandatoryKv();
		kv.mVideoMime = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_MIMETYPE);
		String videoBitRateStr = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_BITRATE);
		kv.mVideoBitRate = Integer.parseInt(videoBitRateStr);
		String widthStr = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_WIDTH);
		kv.mWidth = Integer.parseInt(widthStr);
		String heightStr = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_VIDEO_HEIGHT);
		kv.mHeight = Integer.parseInt(heightStr);
		if (Build.VERSION.SDK_INT >= 23) {
			String frameRateStr = retriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_CAPTURE_FRAMERATE);
			kv.mFrameRate = Integer.parseInt(frameRateStr);
		}
