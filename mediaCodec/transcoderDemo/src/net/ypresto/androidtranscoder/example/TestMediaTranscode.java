...
public class TestMediaTranscode {
    ...
    private static final String OUTPUT_AUDIO_MIME_TYPE = "audio/mp4a-latm"; // Advanced
									    // Audio
									    // Coding
    private static final int OUTPUT_AUDIO_CHANNEL_COUNT = 2; // Must match the
							     // input stream.
    private static final int OUTPUT_AUDIO_BIT_RATE = 128 * 1024;
    private static final int OUTPUT_AUDIO_AAC_PROFILE = MediaCodecInfo.CodecProfileLevel.AACObjectHE;
    private static final int OUTPUT_AUDIO_SAMPLE_RATE_HZ = 44100; // Must match
								  // the input
								  // stream.
	...
	if (videoCodecInfo == null) {
	    Log.d(TAG, "目标视频格式不支持:" + mOutputVideoMime);
	    return;
	}
	MediaCodecInfo audioCodecInfo = selectCodec(mOutputAudioMime);
	if (audioCodecInfo == null) {
	    Log.d(TAG, "目标音频格式不支持:" + mOutputVideoMime);
	    return;
	}
